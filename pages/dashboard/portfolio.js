...
export default function PortfolioPage() {
  ...
  const {
    ...
    divisions, activeDivision, setActiveDivision, hasDivisions, canSwitchDivision, isAdmin,
        ...
        {!isAdmin && !activeDivision && !loading && identityResolved && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="text-xl font-bold text-navy mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>
              You have not been assigned to a division
            </h2>
            <p className="text-sm text-slate-500 max-w-md leading-relaxed mb-8">
              Ask your team admin to assign you to a division so you can start working. If you are the admin, visit the Team page and select your division.
            </p>
            <Link href="/dashboard/team">
              <Button variant="navy">Go to Team Page</Button>
            </Link>
          </div>
        )}
              ...
              {activeTab === 'sku' && !isConsolidated && (
                <div className="ml-auto pb-2">
                  <ExportButton
                    ...
                    onExport={() => exportPortfolio(skuRows, activePeriod?.label, currSym)}
                  />
                </div>
              )}
            </div>
