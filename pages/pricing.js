...
const FEATURES = [
  ...
  { label: 'Up to 5 seats',                         ess: false, pro: true,  ent: false },
  { label: 'Unlimited seats',                       ess: false, pro: false, ent: true },
  ...
  return (
    <div className={`relative rounded-2xl border p-8 flex flex-col transition-all hover:shadow-lg ${
      ...
      <div className="mb-6">
        ...
        {annual ? (
          <p className="mt-1 text-xs text-slate-500">{formatPrice(price * 12)} billed annually</p>
        ) : (
          <p className="mt-1 text-xs font-semibold" style={{ color: '#0D8F8F' }}>Save {Math.round(ANNUAL_DISCOUNT * 100)}% with annual billing</p>
        )}
