 ...
 *
 * Number conventions:
 *   Currency amts → Math.round + toLocaleString('en-US')  e.g. "4,337,514"
 *                   (cells carry the number only; the active currency symbol is
 *                    shown in the column header, e.g. "Price (NZ$)")
 *   Percentages  → +toFixed(1) numeric                   e.g. 23.5
 *   Volumes      → toLocaleString('en-US')               e.g. "12,000"
 *   Booleans     → "Yes" / "No"
 *
 * Currency:
...
/** Round and comma-format a monetary value as a string. Currency-neutral: the
 *  symbol is carried in the column header, not the cell. */
...
export async function exportP1PricingGap(sorted, periodLabel = '', unitId = 'Product ID', currencySym = '₦') {
  const cols = [
    ...
    { header: `Price (${currencySym})`,   width: 20, value: r => fmtCurrency(r.price) },
    { header: 'Margin %',            width: 18, value: r => fmtPct(r.marginPct) },
    { header: 'Comp Gap (%)',        width: 18, value: r => r.compGap != null ? fmtPct(r.compGap) : '' },
    { header: `WTP Gap (${currencySym})`, width: 20, value: r => fmtCurrency(r.wtpGap) },
    { header: `Delta (${currencySym})`,   width: 20, value: r => fmtCurrency(r.delta) },
...
export async function exportP2CostPassThrough(sorted, periodLabel = '', unitId = 'Product ID', currencySym = '₦') {
  const cols = [
    { header: unitId,                      width: 35, value: r => r.sku      ?? '' },
    { header: 'Category',                  width: 22, value: r => r.category ?? '' },
    { header: `Cost Shock (${currencySym})`,    width: 22, value: r => fmtCurrency(r.shock) },
    { header: 'Pass-Through Rate (%)',     width: 24, value: r => r.pt != null ? fmtPct(r.pt * 100) : '' },
    { header: `Absorbed (${currencySym})`,      width: 22, value: r => fmtCurrency(r.absorbed) },
    { header: `FX Absorbed (${currencySym})`,   width: 22, value: r => fmtCurrency(r.fxAbsorbed) },
...
export async function exportP3ChannelEconomics(sorted, periodLabel = '', currencySym = '₦') {
  const cols = [
    ...
    { header: `Revenue (${currencySym})`,            width: 22, value: c => fmtCurrency(c.rev) },
    { header: `Contribution Margin (${currencySym})`, width: 28, value: c => fmtCurrency(c.contMargin) },
...
export async function exportP4TradeExecution(sorted, periodLabel = '', unitId = 'Product ID', currencySym = '₦') {
  const cols = [
    ...
    { header: `Net Impact (${currencySym})`, width: 22, value: r => fmtCurrency(r.netImpact) },
...
export async function exportActions(filtered, currencySym = '₦') {
  const cols = [
    ...
    { header: `Monthly Impact (${currencySym})`, width: 26, value: a => a.value != null ? fmtCurrency(Math.abs(a.value)) : '' },
...
export async function exportPortfolio(skuRows, periodLabel = '', currencySym = '₦') {
  const cols = [
    ...
    { header: 'Category',                  width: 22, value: r => r.category            ?? '' },
    { header: `RRP (${currencySym})`,            width: 18, value: r => fmtCurrency(r.rrp) },
    { header: 'Segment',                   width: 18, value: r => r.segment             ?? '' },
    { header: `COGS/Unit (${currencySym})`,      width: 20, value: r => fmtCurrency(r.cogs_per_unit) },
    { header: `COGS Prior Period (${currencySym})`, width: 26, value: r => fmtCurrency(r.cogs_prior_period) },
    { header: 'COGS Inflation Rate (%)',   width: 24, value: r => r.cogs_inflation_rate    != null ? fmtPct(r.cogs_inflation_rate * 100)    : '' },
    { header: 'Primary Channel',           width: 22, value: r => r.primary_channel        ?? '' },
    { header: 'Distributor Margin (%)',    width: 24, value: r => r.distributor_margin_pct != null ? fmtPct(r.distributor_margin_pct * 100) : '' },
    { header: 'Monthly Volume (Units)',    width: 24, value: r => r.monthly_volume_units    != null ? Number(r.monthly_volume_units).toLocaleString('en-US') : '' },
