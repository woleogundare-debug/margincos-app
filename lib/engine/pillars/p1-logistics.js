...
export function computeLogisticsP1(active) {
  ...
  active.forEach(r => {
    ...
    const el       = nOrNull(r.rate_sensitivity) ?? -0.7;  // default only when missing; respect an explicit 0
    const pc       = n(r.proposed_rate_change_pct);  // template-decimal 0..1
    const wtp      = n(r.rate_headroom_pct);         // template-decimal 0..1
    const compPrice = n(r.market_rate_ngn);
    const floorPct  = n(r.min_margin_floor_pct);     // template-decimal 0..1
    ...
    const backhaulRecovery = n(r.backhaul_recovery_pct);  // template-decimal 0..1
