...
export function computeP2(active) {
  ...
  active.forEach(r => {
    ...
    const fxExposure  = n(r.fx_exposure_pct);  // template-decimal 0..1
    ...
    const inf = (priorCogs > 0 && cogs > 0)
      ...
      : n(r.cogs_inflation_rate);  // template-decimal 0..1

    const pt = n(r.pass_through_rate);  // template-decimal 0..1
