...
function computeLogisticsActions(p1, p2, p3, p4, m1, m2, m3, m4, currencySym = '₦') {
  const actions = [];
  ...
  if (m2 && m2.worstCase < 0) {
    ...
    const detailLines = [
      `Zero fuel-cost pass-through projects ${erosionM}M ${currencySym} margin erosion under a 40% fuel inflation scenario` +
 ...
 */
export function runLogisticsAnalysis(rows, commercialInvestment = [], currencySym = '₦') {
  ...
  const actions = computeLogisticsActions(p1, p2, p3, p4, m1, m2, m3, m4, currencySym);
