...
function computeLogisticsActions(p1, p2, p3, p4, m1, m2, m3, m4) {
  ...
  if (p1.floorBreaches?.length) {
    actions.push({
      ...
      title:  `${p1.floorBreaches.length} lane${p1.floorBreaches.length > 1 ? 's' : ''} breaching minimum margin floor`,
      detail: p1.floorBreaches.slice(0, 3).map(r => r.sku).join(', ') + ' — current gross margin below defined floor',
      value:  0,
      timeline: 'Immediate',
    });
  }
