...
function computeActions(p1, p2, p3, p4, m1, m2, m3, m4, currencySym = '₦') {
  const actions = [];
  ...
  if (upricedSKUs.length) {
    actions.push({
      ...
      _division: getDivisionTag(upricedSKUs),
    });
  }
  ...
  const highAbsorb = (p2.results || [])
    .filter(s => !s.dataMissing && s.absorbedPct > 50)
  ...
  if (badPromos.length) {
    actions.push({
      ...
      timeline: 'This week',
      _division: getDivisionTag(badPromos),
    });
  }
  ...
  if (m1) {
    ...
    if (repriceSKUs.length) {
      actions.push({
        ...
        timeline: 'Within 30 days',
        _division: getDivisionTag(repriceSKUs),
      });
    }
    ...
    if (reviewSKUs.length) {
      actions.push({
        ...
        timeline: 'Within 14 days',
        _division: getDivisionTag(reviewSKUs),
      });
    }
  }

  // ── M2: Forward Scenario Engine ──────────────────────────────────────────
  // Single action: one recoverable pool (40% inflation x 0% pass-through),
  ...
  if (m2 && m2.worstCase < 0) {
    ...
    const detailLines = [
      `Zero cost pass-through projects ${erosionM}M ${currencySym} margin erosion under a 40% inflation scenario` +
        (compressionPp ? ` (${compressionPp}pp margin compression).` : '.'),
      'Sequenced response: (1) immediate - set RRP floors to protect downside; (2) within 30 days - introduce contractual price escalation clauses before next supplier review.',
    ];

    actions.push({
      pillar: 'M2', color: '#C0392B', priority: 0,
      title:  'Margin protection under 40% inflation scenario',
      detail: detailLines.join(' '),
      value:    Math.abs(m2.worstCase),
      timeline: 'Immediate',
      recoverable_pool_id: 'm2-inflation-40pct-0pt',
    });
  }
...
export function runFullAnalysis(rows, tradeInvestment = [], vertical, currencySym = '₦') {
  if (!rows || rows.length === 0) return null;

  // Dispatch to Logistics engine when vertical is Logistics
  if (vertical === 'Logistics') {
    return runLogisticsAnalysis(rows, tradeInvestment, currencySym);
  }
  ...
  const m1      = computeM1(active, base, currencySym);
  const m2      = computeM2(active, base);
  const m3      = computeM3(tradeInvestment, p3);
  const m4      = computeM4(active, base, currencySym);
  const actions = computeActions(p1, p2, p3, p4, m1, m2, m3, m4, currencySym);
