...
function computeActions(p1, p2, p3, p4, m1, m2, m3, m4) {
  ...
  if (m1) {
    ...
    if (repriceSKUs.length) {
      actions.push({
        pillar: 'SKU Rationalisation', color: '#1B2A4A', priority: 1,
    ...
    if (reviewSKUs.length) {
      actions.push({
        pillar: 'SKU Rationalisation', color: '#1B2A4A', priority: 2,
  ...
  if (m2) {
    ...
    if (m2.worstCase < 0) {
      actions.push({
        pillar: 'Inflation Resilience', color: '#C0392B', priority: 0,
    ...
    if (worstScenario && worstScenario.pctPtChange < -20) {
      actions.push({
        pillar: 'Inflation Resilience', color: '#C0392B', priority: 1,
  ...
  if (m3?.hasData) {
    ...
    if (dilutiveChannels.length) {
      ...
      actions.push({
        pillar: 'Trade Spend ROI', color: '#0D8F8F', priority: 1,
  ...
  if (m4?.hasData) {
    ...
    if (renegotiateDist.length) {
      actions.push({
        pillar: 'Distributor Performance', color: '#D4A843', priority: 1,
    ...
    if (exitDist.length) {
      actions.push({
        pillar: 'Distributor Performance', color: '#D4A843', priority: 0,
