...
const SECTOR_CONFIGS = {
  FMCG: {
    ...
    // Download template
    templateFile: '/downloads/MarginCOS_Data_Template_v4_FMCG.xlsx',
    templateFilename: 'MarginCOS_Data_Template_v4_FMCG.xlsx',
    ...
    narrative: {
      ...
      p4Activity:      'promotional activity',
      p4ItemName:      'promo',
      p4NoData:        'No promotional data captured.',
      p4AllProfitable: 'All promotions generating positive margin contribution.',
      p4LossMaking:    (n) => `${n} promotion${n > 1 ? 's' : ''} are loss-making — actual volume lift does not justify discount depth. Recommend capping depth or increasing co-funded trade investment from distributors.`,
    ...
    // Commercial investment form config
    commercialInvestment: {
      tableName:    'trade_investment',
      channelField: 'channel',
      formTitle:    'Monthly Trade Spend by Channel',
      formSubtitle: 'Enter spend in Naira (₦) per category per channel',
      channels: [
...
SECTOR_CONFIGS.Logistics = {
  ...
  templateFile: '/downloads/MarginCOS_Data_Template_v4_Logistics.xlsx',
  templateFilename: 'MarginCOS_Data_Template_v4_Logistics.xlsx',
  ...
  narrative: {
    ...
    p4Activity:      'discounting activity',
    p4ItemName:      'discount',
    p4NoData:        'No discounting data captured.',
    p4AllProfitable: 'All discounts generating positive margin contribution.',
    p4LossMaking:    (n) => `${n} discount${n > 1 ? 's' : ''} are loss-making — actual volume response does not justify the discount depth. Recommend renegotiating volume terms with customers.`,
    benchmarkNarrative: (pct) => {
  ...
  // Commercial investment form config
  commercialInvestment: {
    tableName:    'logistics_commercial_investment',
    channelField: 'contract_type',
    formTitle:    'Monthly Commercial Investment by Contract Type',
    formSubtitle: 'Enter commercial concessions in Naira (\u20a6) per contract type',
    channels: [
      { code: 'CON', label: 'Contracted' },
      { code: 'SPT', label: 'Spot' },
      { code: 'DED', label: 'Dedicated' },
      { code: 'ADH', label: 'Ad-hoc' },
      { code: 'BKH', label: 'Backhaul' },
      { code: 'IML', label: 'Inter-modal' },
    ],
    spendCategories: [
      { key: 'volume_rebates',           label: 'Volume Rebates',          tip: 'Per-trip discount for volume commitments' },
      { key: 'fuel_surcharge_waivers',   label: 'Fuel Surcharge Waivers',  tip: 'Un-collected fuel surcharge revenue' },
      { key: 'rate_holddowns',           label: 'Rate Holddowns',          tip: 'Gap between current rate and cost-adjusted rate' },
      { key: 'deadhead_absorption',      label: 'Deadhead Absorption',     tip: 'Cost of empty return legs not charged to customer' },
      { key: 'credit_extension_cost',    label: 'Credit Extension Cost',   tip: 'Working capital cost of payment terms beyond 30 days' },
      { key: 'fleet_dedication_premium', label: 'Fleet Dedication Premium', tip: 'Opportunity cost of exclusive truck assignment' },
    ],
  },
...
// Manufacturing inherits FMCG analytics but uses generic "product" vocabulary.
// "SKU" is FMCG industry shorthand — Manufacturing clients think in products/parts,
// so the dashboard, PDFs, and charts must speak their language.
// Retail spreads from Manufacturing (same product vocabulary, different template file).
SECTOR_CONFIGS.Manufacturing = {
  ...SECTOR_CONFIGS.FMCG,

  // Unit terminology - product language instead of SKU
  unit: 'Product',
  unitPlural: 'Products',
  unitId: 'Product ID',
  unitName: 'Product',
  ...
  portfolioTab: 'Product Portfolio',
  addButtonLabel: 'Add Product',

  // Narrative fragments - product language
  narrative: {
    ...SECTOR_CONFIGS.FMCG.narrative,
    emptyState: 'Add products in Portfolio Manager to run your first margin analysis.',
    planLimit: (tier, limit) => `Your ${tier} plan supports up to ${limit} active products. Upgrade to unlock more.`,
    periodHint: 'Each period represents a point-in-time snapshot of your portfolio. Create one to begin entering product data.',
    p1FloorBreach: (n) => `${n} product${n !== 1 ? 's' : ''} below margin floor`,
    p2EmptyState: 'Requires cost, inflation rate, and pass-through rate for each product.',
    p3EmptyState: 'Requires primary channel, partner margin, and monthly volume for each product.',
    p4EmptyState: 'Requires promo depth % and promo lift % on products with promotional activity.',
    m4EmptyState: 'No partner data. Populate the partner name field on products to unlock this module.',
  },

  // PDF report - product language
  pdf: {
    ...SECTOR_CONFIGS.FMCG.pdf,
    introLine: (n) => `A pricing intelligence analysis across your active product portfolio of ${n} products.`,
    summaryLine: (n) => `This report analyses ${n} active products across all eight analytical engines.`,
    m1Subtitle: (n, margin) => `${n} dilutive products | Margin at stake: ${margin}`,
    footerLine: (n, date) => `${n} active products | Generated ${date}`,
  },

  // Chart labels - product language
  charts: {
    ...SECTOR_CONFIGS.FMCG.charts,
    p1Title: 'Repricing Opportunity by Product',
    p3Tooltip: 'Products:',
    p4Title: 'Promotion Net Impact by Product',
    m1Subtitle: 'Revenue share vs margin % · Each dot = 1 product',
  },

  // Download template
  templateFile:     '/downloads/MarginCOS_Data_Template_v4_Manufacturing.xlsx',
  templateFilename: 'MarginCOS_Data_Template_v4_Manufacturing.xlsx',
};

// Retail shares Manufacturing's product vocabulary, differs only on template file.
SECTOR_CONFIGS.Retail = {
  ...SECTOR_CONFIGS.Manufacturing,
  templateFile:     '/downloads/MarginCOS_Data_Template_v4_Retail.xlsx',
  templateFilename: 'MarginCOS_Data_Template_v4_Retail.xlsx',
};

// Allowed sector values — single source of truth for validation in
// create-client API, admin form, and database CHECK constraint.
export const ALLOWED_SECTORS = ['FMCG', 'Manufacturing', 'Retail', 'Logistics'];
