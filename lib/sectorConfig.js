...
const SECTOR_CONFIGS = {
  FMCG: {
    ...
    // Download template
    templateFile: '/downloads/MarginCOS_Sample_Template_FMCG.xlsx',
    templateFilename: 'MarginCOS_Sample_Template_FMCG.xlsx',
  ...
  // Manufacturing and Retail inherit from FMCG with minor overrides
  Manufacturing: null,  // Will use FMCG config (see getSectorConfig below)
  Retail: null,         // Will use FMCG config (see getSectorConfig below)
};

// Getter — returns the config for a given sector.
// Manufacturing and Retail use FMCG config (identical engine, identical vocabulary).
...
export function getSectorConfig(sector) {
  ...
  // Manufacturing and Retail fall back to FMCG
  if (['Manufacturing', 'Retail', 'FMCG'].includes(key)) return SECTOR_CONFIGS.FMCG;
  // Unknown sector — safe fallback
